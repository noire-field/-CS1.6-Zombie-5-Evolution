...
#define PLUGIN "[Mileage] Primary: Rail Cannon"
#define VERSION "1.0"
#define AUTHOR "Dias"

#define DAMAGE 122 // 161 for Zombie
...
#define MODEL_V "models/mileage_wpn/pri/v_railcannon.mdl"
...
public plugin_init()
{
	...
	g_RC = Mileage_RegisterWeapon("railcannon")
}
...
{
	if(ItemID == g_RC) Get_RailCannon(id)
}

public Mileage_WeaponRemove(id, ItemID)
{
	if(ItemID == g_RC) Remove_RailCannon(id)
}
...
public Event_CurWeapon(id)
{
	...
	} else if((CSWID == CSW_RAILCANNON && g_OldWeapon[id] == CSW_RAILCANNON) && Get_BitVar(g_Had_RC, id)) {
		static Ent; Ent = fm_get_user_weapon_entity(id, CSW_RAILCANNON)
		if(!pev_valid(Ent))
		{
	...
	} else if(CSWID != CSW_RAILCANNON && g_OldWeapon[id] == CSW_RAILCANNON) {
		if(SubModel != -1) Draw_NewWeapon(id, CSWID)
	}
...
{
	if(CSW_ID == CSW_RAILCANNON)
	{
		static ent
		ent = fm_get_user_weapon_entity(id, CSW_RAILCANNON)
		
		if(pev_valid(ent) && Get_BitVar(g_Had_RC, id))
		{
...
public fw_Weapon_Reload_Post(ent)
{
	...
	if(Get_BitVar(g_Had_RC, id))
	{
		...
		set_pdata_float(ent, 48, RELOAD_TIME + 0.5, 4)
		set_pdata_float(ent, 46, RELOAD_TIME + 0.25, 4)
		set_pdata_float(ent, 47, RELOAD_TIME + 0.25, 4)
