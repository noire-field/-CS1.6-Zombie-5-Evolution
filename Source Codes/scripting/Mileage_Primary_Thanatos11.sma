...
#define PLUGIN "[Mileage] Primary: Thanatos-11"
#define VERSION "1.0"
#define AUTHOR "Dias Pendragon"

#define DAMAGE_A 70 // 140 for zombie, 70 for human
#define DAMAGE_B 260 // 560 for zombie, 280 for human
...
#define V_MODEL "models/mileage_wpn/pri/v_thanatos11_fix.mdl"
#define P_MODEL "models/mileage_wpn/pri/p_primary.mdl"
#define W_MODEL "models/mileage_wpn/pri/w_primary.mdl"
#define S_MODEL "models/mileage_wpn/pri/thanatos11_scythe.mdl"
...
public plugin_init()
{
	...
	g_Thanatos11 = Mileage_RegisterWeapon("thanatos11")
}
...
{
	if(ItemID == g_Thanatos11) Get_Thanatos11(id)
}

public Mileage_WeaponRefillAmmo(id, ItemID)
{
	if(ItemID == g_Thanatos11) cs_set_user_bpammo(id, CSW_THANATOS11, BPAMMO)
}

public Mileage_WeaponRemove(id, ItemID)
{
	if(ItemID == g_Thanatos11) Remove_Thanatos11(id)
}
...
public Reset_Now(id)
{
	if(is_player(id, 1)) update_specialammo(id, g_ChargedAmmo2[id], 0)
}
...
public Event_CurWeapon(id)
{
	...
	} else if((CSWID == CSW_THANATOS11 && g_OldWeapon[id] == CSW_THANATOS11) && Get_BitVar(g_Had_Thanatos11, id)) {
		static Ent; Ent = fm_get_user_weapon_entity(id, CSW_THANATOS11)
		if(!pev_valid(Ent))
		{
	...
	} else if(CSWID != CSW_THANATOS11 && g_OldWeapon[id] == CSW_THANATOS11) {
		if(SubModel != -1) Draw_NewWeapon(id, CSWID)
	}
...
{
	if(CSW_ID == CSW_THANATOS11)
	{
		static ent
		ent = fm_get_user_weapon_entity(id, CSW_THANATOS11)
		
		if(pev_valid(ent) && Get_BitVar(g_Had_Thanatos11, id))
		{
...
public fw_CmdStart(id, uc_handle, seed)
{
	...
	if(get_gametime() - SCYTHE_RELOAD > ReloadTime[id])
	{
		if(g_ChargedAmmo2[id] < SCYTHE_MAX)
		{
			...
			emit_sound(id, CHAN_ITEM, WeaponSounds[5], VOL_NORM, ATTN_NORM, 0, PITCH_NORM)
...
public Shoot_Scythe(id)
{
	...
	{	
		...
		emit_sound(id, CHAN_WEAPON, WeaponSounds[2], 1.0, 0.4, 0, 94 + random_num(0, 15))
	}
...
{
	IG_PlayerAttachment(id, SCYTHE_HEAD, 3.0, 1.0, 10.0)
	IG_PlayerAttachment(id, SCYTHE_CIRCLE, 3.0, 1.0, 2.0)
...
public Explosion(ArraySuck[], taskid)
{
	...
	emit_sound(id, CHAN_ITEM, WeaponSounds[7], VOL_NORM, ATTN_NORM, 0, PITCH_NORM)
