...
#define PLUGIN "[Mileage] Melee: Dragon Sword"
#define VERSION "2.0"
#define AUTHOR "Dias Leon"

#define V_MODEL "models/mileage_wpn/melee/v_dragonsword2.mdl"
#define P_MODEL "models/mileage_wpn/melee/p_dragonsword.mdl"
...
#define SLASH_AHEAD_DAMAGE 550.0
...
new g_DragonSword
new g_Had_Sword, g_Slashing_Mode[33], g_Attack_Mode[33], g_Checking_Mode[33], g_Hit_Ing[33]
...
public plugin_init()
{
	...
	g_DragonSword = Mileage_RegisterWeapon("dragonsword")
}
...
public Event_CurWeapon(id)
{
	...
	if((read_data(2) == CSW_DRAGONSWORD && g_Old_Weapon[id] != CSW_DRAGONSWORD) && Get_BitVar(g_Had_Sword, id))
	{
		set_pev(id, pev_viewmodel2, V_MODEL)
		set_pev(id, pev_weaponmodel2, P_MODEL)
...
public fw_EmitSound(id, channel, const sample[], Float:volume, Float:attn, flags, pitch)
{
	...
	if(/*get_user_weapon(id) != CSW_DRAGONSWORD || */!Get_BitVar(g_Had_Sword, id))
		return FMRES_IGNORED
...
public fw_PlayerTraceAttack(Victim, Attacker, Float:Damage, Float:Direction[3], TraceResult, DamageBits) 
{
	if(!is_user_alive(Attacker))	
		return HAM_IGNORED
	if(!Get_BitVar(g_Had_Sword, Attacker) || !g_Checking_Mode[Attacker])
		return HAM_IGNORED
