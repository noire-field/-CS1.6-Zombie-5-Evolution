...
#define PLUGIN "[Mileage] Primary: Compound Bow"
#define VERSION "1.0"
#define AUTHOR "Dias Leon"

// =========== Resources Config
#define V_MODEL "models/mileage_wpn/pri/v_bow.mdl"
#define P_MODEL "models/mileage_wpn/pri/p_primary.mdl"
#define W_MODEL "models/mileage_wpn/pri/w_primary.mdl"
#define SUBMODEL 15

#define ARROW_MODEl "models/mileage_wpn/pri/arrow.mdl"
...
public plugin_init()
{
	...
	g_Bow = Mileage_RegisterWeapon("bow")
...
{
	if(ItemID == g_Bow) g_BowArrow[id] = ARROW_DEFAULT
}
...
public Event_CurWeapon(id)
{
	...
	if((CSWID == CSW_BOW && g_OldWeapon[id] != CSW_BOW) && Get_BitVar(g_Had_CompoundBow, id))
	{
		if(SubModel != -1) Draw_NewWeapon(id, CSWID)
		UpdateAmmo(id, CSW_BOW, BOW_AMMOID, -1, g_BowArrow[id])
	} else if((CSWID == CSW_BOW && g_OldWeapon[id] == CSW_BOW) && Get_BitVar(g_Had_CompoundBow, id)) {
		UpdateAmmo(id, CSW_BOW, BOW_AMMOID, -1, g_BowArrow[id])
	} else if(CSWID != CSW_BOW && g_OldWeapon[id] == CSW_BOW) {
		if(SubModel != -1) Draw_NewWeapon(id, CSWID)
	}
...
public Draw_NewWeapon(id, CSW_ID)
{
	...
	{
		static ent
		ent = fm_get_user_weapon_entity(id, CSW_BOW)
		
		if(pev_valid(ent) && Get_BitVar(g_Had_CompoundBow, id))
		{
...
public Bow_NormalAttackHandle(id, UnCharge)
{
	...
	emit_sound(id, CHAN_WEAPON, WeaponSounds[UnCharge ? 1 : 0], 1.0, 0.4, 0, 94 + random_num(0, 15))
