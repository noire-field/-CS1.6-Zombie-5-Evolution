...
#define PLUGIN "[Mileage] Primary: Salamander"
#define VERSION "2015"
...
#define MODEL_V "models/mileage_wpn/pri/v_salamander.mdl"
#define MODEL_P "models/mileage_wpn/pri/p_primary.mdl"
#define MODEL_W "models/mileage_wpn/pri/w_primary.mdl"
...
// Vars
new g_Salamander, g_OldWeapon[33]
...
public plugin_init()
{
	...
	g_Salamander = Mileage_RegisterWeapon("salamander")
...
public plugin_precache()
{
	...
	{
		if(i == 2) g_SmokePuff_SprID = precache_model(WeaponResources[i])
...
{
	if(ItemID == g_Salamander) cs_set_user_bpammo(id, CSW_SAL, 200)
}
...
public Event_CurWeapon(id)
{
	...
	if((CSWID == CSW_SAL && g_OldWeapon[id] != CSW_SAL) && Get_BitVar(g_Had_Salamander, id))
	{
		if(SubModel != -1) Draw_NewWeapon(id, CSWID)
	} else if((CSWID == CSW_SAL && g_OldWeapon[id] == CSW_SAL) && Get_BitVar(g_Had_Salamander, id)) {
		
	} else if(CSWID != CSW_SAL && g_OldWeapon[id] == CSW_SAL) {
		if(SubModel != -1) Draw_NewWeapon(id, CSWID)
	}
...
public Draw_NewWeapon(id, CSW_ID)
{
	...
	{
		static ent
		ent = fm_get_user_weapon_entity(id, CSW_SAL)
		
		if(pev_valid(ent) && Get_BitVar(g_Had_Salamander, id))
		{
...
public CreateFire(id, Float:Speed)
{
	...
	emit_sound(iEnt, CHAN_BODY, WeaponSounds[0], 1.0, 0.4, 0, 94 + random_num(0, 15))
}
