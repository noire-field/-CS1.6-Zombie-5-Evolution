...
#define PLUGIN "[Mileage] Secondary: Cyclone"
#define VERSION "1.0"
...
#define V_MODEL "models/mileage_wpn/sec/v_sfpistol.mdl"
...
new g_Cyclone
new g_Had_Cyclone, g_Beam_SprId, g_Burn_SprId, g_OldWeapon[33]
...
public plugin_init() 
{
	...
	g_Cyclone = Mileage_RegisterWeapon("cyclone")
...
public plugin_precache()
{
	...
	g_Burn_SprId = engfunc(EngFunc_PrecacheModel, WeaponResources[0])
...
{
	if(ItemID == g_Cyclone) cs_set_user_bpammo(id, CSW_CYCLONE, BPAMMO)
}
...
public Event_CurWeapon(id)
{
	...
	if((CSWID == CSW_CYCLONE && g_OldWeapon[id] != CSW_CYCLONE) && Get_BitVar(g_Had_Cyclone, id))
	{
		Draw_NewWeapon(id, CSWID)
	} else if((CSWID == CSW_CYCLONE && g_OldWeapon[id] == CSW_CYCLONE) && Get_BitVar(g_Had_Cyclone, id)) {
		
	} else if(CSWID != CSW_CYCLONE && g_OldWeapon[id] == CSW_CYCLONE) Draw_NewWeapon(id, CSWID)
...
{
	if(CSW_ID == CSW_CYCLONE)
	{
		static ent
		ent = fm_get_user_weapon_entity(id, CSW_CYCLONE)
		
		if(pev_valid(ent) && Get_BitVar(g_Had_Cyclone, id))
		{
...
public Cyclone_AttackHandle(id, Pass)
{
	...
	switch(g_WeaponState[id])
	{
		...
		{
			if(pev(id, pev_weaponanim) != CYCLONE_ANIM_SHOOT) Set_WeaponAnim(id, CYCLONE_ANIM_SHOOT)
