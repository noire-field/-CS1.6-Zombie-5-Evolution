...
new g_Janus1, g_OldWeapon[33]
new g_Had_Janus1, g_Janus1_Mode[33], g_ChangingMode, g_expspr_id , g_SmokeSprId
new g_Event_Janus1, g_ham_bot, g_MaxPlayers, spr_trail
...
public plugin_init()
{
	...
	g_Janus1 = Mileage_RegisterWeapon("janus1")
...
public Event_NewRound() remove_entity_name("grenade2")

public Event_CurWeapon(id)
{
	static CSWID; CSWID = read_data(2)
	
	if((CSWID == CSW_DEAGLE && g_OldWeapon[id] != CSW_DEAGLE) && Get_BitVar(g_Had_Janus1, id))
	{
		engfunc(EngFunc_MessageBegin, MSG_ONE_UNRELIABLE, get_user_msgid("CurWeapon"), {0, 0, 0}, id)
		write_byte(1)
		write_byte(CSW_Janus1)
	...
	} else if((CSWID == CSW_DEAGLE && g_OldWeapon[id] == CSW_DEAGLE) && Get_BitVar(g_Had_Janus1, id)) {
		engfunc(EngFunc_MessageBegin, MSG_ONE_UNRELIABLE, get_user_msgid("CurWeapon"), {0, 0, 0}, id)
		write_byte(1)
		write_byte(CSW_Janus1)
		write_byte(-1)
		message_end()
	} else if(CSWID != CSW_DEAGLE && g_OldWeapon[id] == CSW_DEAGLE) Draw_NewWeapon(id, CSWID)
...
{
	if(CSW_ID == CSW_DEAGLE)
	{
		static ent
		ent = fm_get_user_weapon_entity(id, CSW_DEAGLE)
		
		if(pev_valid(ent) && Get_BitVar(g_Had_Janus1, id))
		{
...
public fw_CmdStart(id, uc_handle, seed)
{
	...
	if((PressButton & IN_ATTACK))
	{
		...
		{ // Normal
			...
			emit_sound(id, CHAN_WEAPON, Janus1_Sounds[0], 1.0, 0.4, 0, 94 + random_num(0, 15))
			...
			emit_sound(id, CHAN_WEAPON, Janus1_Sounds[1], 1.0, 0.4, 0, 94 + random_num(0, 15))
