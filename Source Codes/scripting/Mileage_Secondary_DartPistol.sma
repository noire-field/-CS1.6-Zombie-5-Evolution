...
#define PLUGIN "[Mileage] Secondary: Dart Pistol"
#define VERSION "1.0"
#define AUTHOR "Dias"
...
#define MODEL_V "models/mileage_wpn/sec/v_dartpistol.mdl"
#define MODEL_P "models/mileage_wpn/sec/p_secondary.mdl"
#define MODEL_W "models/mileage_wpn/sec/w_secondary.mdl"
#define MODEL_S "models/mileage_wpn/sec/dart.mdl"
...
// Vars
new g_DartPistol
new g_Had_DartPistol, g_Clip[33], g_OldWeapon[33]
...
public plugin_init()
{
	...
	g_DartPistol = Mileage_RegisterWeapon("dpistol")
}
...
{
	if(ItemID == g_DartPistol) Get_DartPistol(id)
}

public Mileage_WeaponRefillAmmo(id, ItemID)
{
	if(ItemID == g_DartPistol) cs_set_user_bpammo(id, CSW_DEAGLE, BPAMMO)
}

public Mileage_WeaponRemove(id, ItemID)
{
	if(ItemID == g_DartPistol) Remove_DartPistol(id)
}
...
public Event_NewRound() remove_entity_name(DART_CLASSNAME)

public Event_CurWeapon(id)
{
	static CSWID; CSWID = read_data(2)
	
	if((CSWID == CSW_DEAGLE && g_OldWeapon[id] != CSW_DEAGLE) && Get_BitVar(g_Had_DartPistol, id))
	{
		Draw_NewWeapon(id, CSWID)
	} else if((CSWID == CSW_DEAGLE && g_OldWeapon[id] == CSW_DEAGLE) && Get_BitVar(g_Had_DartPistol, id)) {
		static Ent; Ent = fm_get_user_weapon_entity(id, CSW_DEAGLE)
		if(!pev_valid(Ent))
		{
...
public Draw_NewWeapon(id, CSW_ID)
{
	if(CSW_ID == CSW_DEAGLE)
	{
		...
		if(pev_valid(ent) && Get_BitVar(g_Had_DartPistol, id))
		{
