...
#define PLUGIN "[Mileage] Primary: Aeolis"
#define VERSION "1.0"
#define AUTHOR "Dias no Pendragon"

#define DAMAGE_A 35 // 45 for zombie
#define DAMAGE_B 45 // 60 for zombie
...
#define SUBMODEL 12
#define V_MODEL "models/mileage_wpn/pri/v_spmg.mdl"
...
public plugin_init() 
{
	...
	g_Aeolis = Mileage_RegisterWeapon("aeolis")
}
...
{
	if(ItemID == g_Aeolis) cs_set_user_bpammo(id, CSW_AEOLIS, 250)
}
...
public Reset_Now(id)
{
	if(is_user_alive(id)) Update_SpecialAmmo(id, g_CurrentAmmo[id], 0)
}
...
public Event_CurWeapon(id)
{
	...
	if((CSWID == CSW_AEOLIS && g_OldWeapon[id] != CSW_AEOLIS) && Get_BitVar(g_Had_Aeolis, id))
	{
		if(SubModel != -1) Draw_NewWeapon(id, CSWID)
	} else if((CSWID == CSW_AEOLIS && g_OldWeapon[id] == CSW_AEOLIS) && Get_BitVar(g_Had_Aeolis, id)) {
		
	} else if(CSWID != CSW_AEOLIS && g_OldWeapon[id] == CSW_AEOLIS) {
		if(SubModel != -1) Draw_NewWeapon(id, CSWID)
	}
...
{
	if(CSW_ID == CSW_AEOLIS)
	{
		static ent
		ent = fm_get_user_weapon_entity(id, CSW_AEOLIS)
		
		if(pev_valid(ent) && Get_BitVar(g_Had_Aeolis, id))
		{
...
public Check_FireAmmoHud(id, Active, Shooting)
{
	...
	if(g_FireAmmo[id] >= 10) 
	{
		if(min(Ammo, 9) > g_CurrentAmmo[id])
		{
			...
			if(OK) 
			{
				emit_sound(id, CHAN_ITEM, WeaponSounds[2], 1.0, 0.4, 0, 94 + random_num(0, 15))
				Make_SteamSmoke(id)
			}
