...
#define PLUGIN "[Mileage] Primary: Ethereal"
#define VERSION "1.0"
...
new g_Ethereal
new g_Had_Ethereal, g_Ethereal_Clip[33], Float:g_Recoil[33][3], g_OldWeapon[33]
...
public plugin_init()
{
	...
	g_Ethereal = Mileage_RegisterWeapon("ethereal")
}
...
public Event_CurWeapon(id)
{
	...
	if((CSWID == CSW_ETHEREAL && g_OldWeapon[id] != CSW_ETHEREAL) && Get_BitVar(g_Had_Ethereal, id))
	{
		if(SubModel != -1) Draw_NewWeapon(id, CSWID)
	} else if((CSWID == CSW_ETHEREAL && g_OldWeapon[id] == CSW_ETHEREAL) && Get_BitVar(g_Had_Ethereal, id)) {
	...
	} else if(CSWID != CSW_ETHEREAL && g_OldWeapon[id] == CSW_ETHEREAL) {
		if(SubModel != -1) Draw_NewWeapon(id, CSWID)
	}
...
{
	if(CSW_ID == CSW_ETHEREAL)
	{
		static ent
		ent = fm_get_user_weapon_entity(id, CSW_ETHEREAL)
		
		if(pev_valid(ent) && Get_BitVar(g_Had_Ethereal, id))
		{
...
public fw_PlaybackEvent(flags, invoker, eventid, Float:delay, Float:origin[3], Float:angles[3], Float:fparam1, Float:fparam2, iParam1, iParam2, bParam1, bParam2)
{
	...
	emit_sound(invoker, CHAN_WEAPON, Ethereal_Sounds[0], 1.0, 0.4, 0, 94 + random_num(0, 15))
