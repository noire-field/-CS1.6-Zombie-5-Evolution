...
#define PLUGIN "[Mileage] Secondary: Sapientia"
#define VERSION "1.0"
#define AUTHOR "Dias"

#define DAMAGE 62 // 84 for zombie
#define CLIP 7
#define BPAMMO 210
...
#define MODEL_V "models/mileage_wpn/sec/v_sapientia.mdl"
...
new g_SprID, g_Event_Sapientia, g_SmokePuff_SprId, g_OldWeapon[33]
new g_MsgCurWeapon
...
public plugin_init()
{
	...
	g_Sapientia = Mileage_RegisterWeapon("sapientia")
}
...
public Event_CurWeapon(id)
{
	...
	if((CSWID == CSW_DEAGLE && g_OldWeapon[id] != CSW_DEAGLE) && Get_BitVar(g_Had_Sapientia, id))
	{
		Draw_NewWeapon(id, CSWID)
	} else if((CSWID == CSW_DEAGLE && g_OldWeapon[id] == CSW_DEAGLE) && Get_BitVar(g_Had_Sapientia, id)) {
		static Ent; Ent = fm_get_user_weapon_entity(id, CSW_DEAGLE)
		if(!pev_valid(Ent))
		{
...
public Draw_NewWeapon(id, CSW_ID)
{
	if(CSW_ID == CSW_DEAGLE)
	{
		...
		if(pev_valid(ent) && Get_BitVar(g_Had_Sapientia, id))
		{
...
public fw_TraceAttack_Player(Victim, Attacker, Float:Damage, Float:Direction[3], Ptr, DamageBits)
{
	...
	if(is_connected(Victim) && cs_get_user_team(Attacker) != cs_get_user_team(Victim))
		Make_AfterBurn(Victim, Attacker, 3.0, DAMAGE / 2)
