...
#define PLUGIN "[Mileage] Primary: Thanatos-7"
#define VERSION "1.0"
#define AUTHOR "Dias no Pendragon"

#define DAMAGE_A 37 // 66 for zombie
#define DAMAGE_B 200 // 250 for zombie

#define CLIP 120
#define SCYTHE_LIFETIME 10.0

#define SUBMODEL 13
#define V_MODEL "models/mileage_wpn/pri/v_thanatos7.mdl"
#define P_MODEL "models/mileage_wpn/pri/p_primary.mdl"
#define W_MODEL "models/mileage_wpn/pri/w_primary.mdl"
#define S_MODEL "models/mileage_wpn/pri/thanatos7_scythe.mdl"
...
new g_Thanatos7, g_OldWeapon[33]
...
public plugin_init() 
{
	...
	g_Thanatos7 = Mileage_RegisterWeapon("thanatos7")
}
...
{
	if(ItemID == g_Thanatos7) cs_set_user_bpammo(id, CSW_THANATOS7, 250)
}
...
public Event_CurWeapon(id)
{
	...
	if((CSWID == CSW_THANATOS7 && g_OldWeapon[id] != CSW_THANATOS7) && Get_BitVar(g_Had_Thanatos7, id))
	{
		if(SubModel != -1) Draw_NewWeapon(id, CSWID)
	} else if((CSWID == CSW_THANATOS7 && g_OldWeapon[id] == CSW_THANATOS7) && Get_BitVar(g_Had_Thanatos7, id)) {
		
	} else if(CSWID != CSW_THANATOS7 && g_OldWeapon[id] == CSW_THANATOS7) {
		if(SubModel != -1) Draw_NewWeapon(id, CSWID)
	}
...
{
	if(CSW_ID == CSW_THANATOS7)
	{
		static ent
		ent = fm_get_user_weapon_entity(id, CSW_THANATOS7)
		
		if(pev_valid(ent) && Get_BitVar(g_Had_Thanatos7, id))
		{
